...
public class NameInputScreen implements Screen {
    ...
    public void show() {
        ...
        // Memastikan file logo ada di folder assets utama
        logoTexture = new Texture(Gdx.files.internal("logo.png"));

        // Matematika Tengah: (Layar/2) - (Lebar/2)
        float btnW = 200, btnH = 60;
        float btnX = (SCREEN_SIZE / 2f) - (btnW / 2f); // Hasilnya 300
        float btnY = 250;
        ...
        Gdx.input.setInputProcessor(new InputAdapter() {
            ...
            public boolean touchDown(int x, int y, int p, int b) {
                // Konversi Y karena LibGDX koordinat mouse terbalik
    ...
    public void render(float delta) {
        Gdx.gl.glClearColor(1, 1, 1, 1); // Background Putih
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);

        batch.begin();

        // LOGO: Digambar kotak (400x400) biar gak miring karena file aslinya kotak
        float logoSize = 400;
